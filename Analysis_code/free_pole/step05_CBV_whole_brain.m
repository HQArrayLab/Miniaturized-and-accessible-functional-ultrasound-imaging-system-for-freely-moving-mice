...
dx=0.05;
dz=0.05;
roi_mask=roi_mask(27:150,1:115);
Doppler = Data_average(27:150,1:115,:).*roi_mask;
...
bin = 3;
step = 4;
v = VideoWriter('result\test2.mp4','MPEG-4');  
v.Quality = 95;
v.FrameRate = 2;
open(v);   % 打开AVI文件
...
for k=1+bin:step:size(DopplerN,3)
    ...
    if bin==0
       ...
       Mapsum =sum(Map(:,:,k-bin:k),3)./(bin+1); %(Mapsum+Map(:,:,k));
    end
    ...
    valid_pixels = (Mapsum>0.1)&roi_mask;
    ...
    % 链接坐标轴
    linkaxes([ax1, ax2]);
    addLines(LinReg.Cor,slice,X,Z,0.95,-2.0,0.93,0.25,dx,dz);

    if [k/frame_rate>=20 && k/frame_rate<= 30]
        text(-2.9,0.2,[sprintf(' On pole')],'FontSize',14,'Color','w','FontWeight','bold');
    end

    text(-3.1,5.9,[sprintf('%5d',round(k/frame_rate)),'s'],'FontSize',14,'Color','w','FontWeight','bold');
    set(gcf,'color','white');  
    ...
    text(mean([x1 x2]), line_y+0.4, '1mm', ...
